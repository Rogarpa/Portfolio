...
# queremos que createPostIt reciba array, para poder resizear
...
class Aplication(Frame):
    ...
    def create_toolbar_widgets(self):
        ...
        self.tool_bar, text="Create color postit html photomosaic", command=self.createColorHtmlPhotomosaic)
        self.gray_recursive_button.grid(row=5, column=2, padx=5, pady=3, ipadx=10)
        
        self.color_recursive_button = Button(
        self.tool_bar, text="Create gray postit html photomosaic", command=self.createGrayHtmlPhotomosaic)
    ...
    def reselect_image(self):
        ...
        fp = fd.askopenfilename(
            title='Open a file',
            initialdir ='~/Desktop/PDI/P/demos/',
            filetypes=filetypes)
    ...
    def createGrayHtmlPhotomosaic(self):
        try:
            ...
            self.filters.grayPostitPhotomosaic(
    ...
    def createColorHtmlPhotomosaic(self):
        ...
        try:
            ...
            self.filters.colorPostitPhotomosaic(
                                    self.to_edit_image_fp
                                    ,int(self.mosaic_width_box.get())
                                    ,int(self.mosaic_length_box.get())
                                    )
...
class Filters():
    ...
    def grayPostitPhotomosaic(self, photo_fp, sqOgWidth, sqOgLength):
        post_it_fp = 'postred.jpg'
        ...
        for m in range(rowsOfSquares):
            ...
            for n in range(columnsOfSquares):
                ...
                postIt_image = Image.open('postred.jpg').resize((sqLength,sqWidth))
                postIt_matrix = postIt_image.load()
                
                post_width = postIt_image.size[0]
                post_length = postIt_image.size[1]

                for x in range(post_width):
                    for y in range(post_length):
                        # White or gray
                        if((postIt_matrix[x,y][0] == postIt_matrix[x,y][1])):
                            break
                ...
                postIt_image.save(fp_substitute_mosaic_image)
    ...
    def colorPostitPhotomosaic(self, photo_fp, sqOgWidth, sqOgLength):
        og_image = Image.open(photo_fp)
        width = og_image.size[0]
        length = og_image.size[1]
        array = og_image.load()
