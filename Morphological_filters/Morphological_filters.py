...
class Filters:
    ...
    def max(self, array, width, length):
        self.mean(array, width, length)
        def non_surrounded_of_blacks(r,g,b):
            if(r == 255):
                return (255,255,255)
            else:
                return (0,0,0)
        self.pixelPerPixelFilter(array, width, length, self.non_surrounded_of_blacks)
    def min(self, array, width, length):
        self.mean(array, width, length)
        def non_surrounded_of_whites(r,g,b):
            if(r == 0):
                return (0,0,0)
            else:
                return (255,255,255)
        self.pixelPerPixelFilter(array, width, length, self.non_surrounded_of_whites)
...
class Aplicacion(Frame):
    ...
    def create_toolbar_widgets(self):
        ...
        self.minimum_button = Button(
        self.tool_bar, text="Maximum", command=self.applyMaximum)
        self.minimum_button.grid(row=5, column=0, padx=5, pady=3, ipadx=10)
        
        self.minimum_button = Button(
        self.tool_bar, text="Minimum", command=self.applyMinimum)
        self.minimum_button.grid(row=5, column=1, padx=5, pady=3, ipadx=10)
...
root.title("P4")  # title of the GUI window
